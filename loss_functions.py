...
def compute_photo_and_geometry_loss(tgt_img, ref_imgs, intrinsics, tgt_depth, ref_depths, poses, poses_inv, max_scales, with_ssim, with_mask, with_auto_mask, padding_mode):
    ...
    for ref_img, ref_depth, pose, pose_inv in zip(ref_imgs, ref_depths, poses, poses_inv):
        for s in range(num_scales):
            ...
            photo_loss1, geometry_loss1, valid_mask1 = compute_pairwise_loss(tgt_img_scaled, ref_img_scaled, tgt_depth_scaled, ref_depth_scaled, pose,
                                                                intrinsic_scaled, with_ssim, with_mask, with_auto_mask, padding_mode)
            photo_loss2, geometry_loss2, valid_mask2 = compute_pairwise_loss(ref_img_scaled, tgt_img_scaled, ref_depth_scaled, tgt_depth_scaled, pose_inv,
                                                                intrinsic_scaled, with_ssim, with_mask, with_auto_mask, padding_mode)
            #print(photo_loss1.grad_fn, photo_loss2.grad_fn, geometry_loss1.grad_fn, geometry_loss2.grad_fn)
            photo_loss += (photo_loss1 + photo_loss2)
            geometry_loss += (geometry_loss1 + geometry_loss2)
    #print('{} times hit too low mask points'.format(count_too_low))
    return photo_loss, geometry_loss, valid_mask1, valid_mask2
...
def compute_pairwise_loss(tgt_img, ref_img, tgt_depth, ref_depth, pose, intrinsic, with_ssim, with_mask, with_auto_mask, padding_mode):
    ...
    return reconstruction_loss, geometry_consistency_loss, valid_mask
...
def mean_on_mask(diff, valid_mask):
    ...
    if mask.sum() > 10000:
        ...
        mean_value = (diff * mask).sum() / (mask.sum() + 1)
        #count_too_low += 1
...
def compute_errors(gt, pred, dataset):
    ...
    if dataset == 'mendel' or dataset == 'intel':
        crop_mask = gt[0] != gt[0]
        y1, y2 = int(0 * gt.size(1)), int(1 * gt.size(1))
        x1, x2 = int(0 * gt.size(2)), int(1 * gt.size(2))
        crop_mask[y1:y2, x1:x2] = 1
        max_depth = 10
